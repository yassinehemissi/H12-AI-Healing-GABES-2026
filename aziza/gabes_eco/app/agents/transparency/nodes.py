...
from app.agents.transparency.tools import fetch_pollution_data, fetch_citizen_reports, compute_aqi_from_data
from app.agents.transparency.prompts import ANALYZE_POLLUTION_PROMPT, GENERATE_ALERT_PROMPT, GENERATE_RECOMMENDATIONS_PROMPT, CATEGORIZE_REPORTS_PROMPT
...
async def load_data_node(state: TransparencyState) -> dict:
    ...
    # Inject the user's query as a real-time citizen report
    if user_query:
        import datetime
...
async def analyze_pollution_node(state: TransparencyState) -> dict:
    ...
    try:
        ...
        # We temporarily store pollution_summary in the state as a side effect or process it later.
        # But for now, we just pass what the prompt requested.
        return {
...
async def generate_recommendations_node(state: TransparencyState) -> dict:
    ...
    try:
        ...
        recommendations = ["Prenez des précautions de base."]
...
async def categorize_report_node(state: TransparencyState) -> dict:
    ...
    try:
        ...
        # Merge back descriptions/timestamps
        enriched_reports = []
        for orig in reports:
            # Find matching category from LLM output
...
async def credibility_score_node(state: TransparencyState) -> dict:
    ...
    for report in reports:
        ...
        # Simplification: if multiple reports exist overall, bump score
        if len(reports) > 2:
            score += 0.2
...
# Fallback static coordinates for Gabès
GABES_ZONES = {
...
async def geolocate_node(state: TransparencyState) -> dict:
    ...
    for report in reports:
        ...
        lat, lon = None, None
        try:
            location_data = geolocator.geocode(f"{zone}, Gabès, Tunisia")
            if location_data:
        ...
        # Jitter to avoid overlapping pins
...
async def cluster_reports_node(state: TransparencyState) -> dict:
    ...
    # Toujours préparer le GeoJSON, même s'il y a < 2 points
    features = []
    for pt in points:
        features.append({
            "type": "Feature",
...
async def format_final_response_node(state: TransparencyState) -> dict:
    final_response = {
        "location": state["location"],
        "date": state.get("date", "Dernière date disponible"),
